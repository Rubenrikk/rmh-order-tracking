...
class Printcom_Order_Tracker {
    ...
    public function render_order_shortcode($atts=[]) {
        ...
        if ($provided !== $token) {
            $pc = isset($_POST['rmh_ot_pc']) ? sanitize_text_field(wp_unslash($_POST['rmh_ot_pc'])) : '';
            if ($pc !== '') {
                $data = get_transient($cache_key);
                if (!$data) {
                    $data = $this->api_get_order($orderNum);
                    if (is_wp_error($data)) return '<div class="rmh-ot rmh-ot--error">Order niet gevonden.</div>'.$this->postcode_form();
                    set_transient($cache_key,$data,$this->dynamic_cache_ttl_for($data));
                }
                $addr     = $this->extract_primary_shipping_address($data);
                $order_pc = isset($addr['postalCode']) ? strtoupper(preg_replace('/\s+/','',$addr['postalCode'])) : '';
                $pc_in    = strtoupper(preg_replace('/\s+/','',$pc));
                if ($order_pc === '' || $pc_in !== $order_pc) {
                    return '<div class="rmh-ot rmh-ot--error">Postcode komt niet overeen.</div>'.$this->postcode_form();
                }
            } else {
                return '<div class="rmh-ot rmh-ot--error">Gebruik de link uit de e-mail of vul uw postcode in om uw bestelling te bekijken.</div>'.$this->postcode_form();
            }
        } else {
            $data = get_transient($cache_key);
            if (!$data) {
    ...
    private function postcode_form(): string {
        return '<form method="post" class="rmh-ot__pc-form"><label for="rmh_ot_pc">Postcode</label> <input type="text" id="rmh_ot_pc" name="rmh_ot_pc" required/> <button type="submit">Toon bestelling</button></form>';
    }
